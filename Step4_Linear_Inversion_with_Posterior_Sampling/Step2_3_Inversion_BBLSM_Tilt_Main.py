...
from Linear_Inversion_Tools.Nonnegative_HMC_Posterior_Sampling import hmc_with_nonnegative_projection
from Linear_Inversion_Tools.Nonnegative_HMC_Posterior_Sampling import hmc_posterior_analysis_chi, hmc_posterior_analysis_distance
from Linear_Inversion_Tools.Nonnegative_HMC_Posterior_Sampling import plot_all_selected_hmc_models
...
if __name__ == '__main__':
    ...
    [posterior_model_mean, posterior_model_covariance] = bayesian_bounded_least_square_solution(G=G_joint_tilt, dobs=data_obs_joint_tilt, model_prior=model_prior,
                                                                                                Cd=Cd_joint_tilt, Cm=Cm, bounds=(0, 3e6))
    ...
    plot_posterior_model(R=model_general['R'], num_subs=auxiliary_parameters['num_subs'], posterior_model=posterior_model_mean,
                         title="Bayesian Bounded Least-Squares Solution", file_name="BBLSM_result.png")
    ...
    step_size = 0.003
    max_value = 3e6
    # 9.1) Posterior model sampling us MH algorithm with Non-negative projection
    # initial_sample = posterior_model_mean
    # posterior_model_samples = hmc_with_nonnegative_projection(initial_sample=initial_sample, num_samples=num_samples, leapfrog_steps=leapfrog_steps, step_size=step_size, max_value=max_value,
    ...
    # np.save(f"{num_samples}_BBLSM_HMC_posterior_samples.npy", posterior_model_samples)
    posterior_model_samples = np.load(f"{num_samples}_BBLSM_HMC_posterior_samples.npy")
    sorted_posterior_chi_samples = hmc_posterior_analysis_chi(hmc_accepted_samples=posterior_model_samples, initial_sample=posterior_model_mean, bins=200,
                                                                model_prior=model_prior, Cd=Cd_joint_tilt, Cm=Cm, G=G_joint_tilt, dobs=data_obs_joint_tilt,
                                                                trim=100, file_name="hmc_posterior_chi_values_distribution.png")
    sorted_posterior_dist_samples = hmc_posterior_analysis_distance(hmc_accepted_samples=posterior_model_samples, initial_sample=posterior_model_mean, bins=200,
                                                                    trim=100, file_name="hmc_posterior_distances_distributions.png")

    # 9.2) Visualization of the selected models: first 9 samples
    first_nums = np.arange(0, 9)
    plot_all_selected_hmc_models(R=model_general['R'], num_subs=auxiliary_parameters['num_subs'],
                                 samples=sorted_posterior_dist_samples, selected_indices=first_nums, num_cols=3,
                                 file_name="BNNLSM_HMC_posterior_samples.png")
