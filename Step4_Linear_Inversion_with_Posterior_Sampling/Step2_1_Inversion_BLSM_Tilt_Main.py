...
if __name__ == '__main__':
    ...
    # 7) BLSM solution and visualization
    [posterior_model_mean, posterior_model_covariance] = bayesian_least_square_solution_formula2(G=G_joint_tilt, dobs=data_obs_joint_tilt, Cd=Cd_joint_tilt,
    ...
    plot_posterior_model(R=model_general['R'], num_subs=auxiliary_parameters['num_subs'], posterior_model=posterior_model_mean,
                         title="Posterior Maximum Likelihood Reservoir Model: Mean Values", file_name="posterior_mean_model.png")
    # Visualization 2 (optional): Posterior model and uncertainties of some selected model parameters
    selected_model_indices = {72: {'color': 'red', 'label': '72'},
                              81: {'color': 'blue', 'label': '81'},
                              265: {'color': 'green', 'label': '265'}}
    plot_posterior_and_uncertainty(R=model_general['R'], num_subs=auxiliary_parameters['num_subs'],
                                   prior_model_cov=Cm, prior_model=model_prior,
                                   posterior_model=posterior_model_mean, posterior_model_cov=posterior_model_covariance,
                                   selected_model_indices=selected_model_indices, file_name="posterior_model_with_uncertainties.png")
    # Visualization 3 (optional): 2 model parameters joint and marginal in prior and posterior distributions
    plot_joint_marginal_prior_posterior_comparison(prior_mean=model_prior, prior_cov=Cm,
                                                   posterior_mean=posterior_model_mean, posterior_cov=posterior_model_covariance,
                                                   index1=72, index2=97, file_name="joint_marginal_prior_posterior_comparison.png")

    ###########################################################################
    ############# Optional 1: Posterior Sampling and Visualization   ##########
    ############# Method: Cholesky decomposition posterior sampling  ##########
    ###########################################################################
    # 8) Posterior model sampling and visualization
    num_samples = 10000
    random_seed = 45
    PCA_components = 2       # The number of PCA components for visualization, 2(D) is the best for display.
    num_selected_models = 3  # The number of selected models (per range / group / row) for visualization.
    # 8.1) Posterior model sampling
    posterior_model_samples, distances = posterior_sampling(posterior_model_cov=posterior_model_covariance, posterior_model_mean=posterior_model_mean,
                                                            num_samples=num_samples, random_seed=random_seed)
    # 8.2) PCA transformation
    samples_pca, mean_pca, cov_pca, std_dev_pca = pca_transformation(model_samples=posterior_model_samples, n_components=PCA_components)
    # 8.3) Select samples within custom ranges (related to std)
    custom_std_ranges = np.array([[0, 0.5], [0.5, 1.0], [1.0, 2.0]]) * np.mean(std_dev_pca)
    selected_indices = select_samples_within_custom_ranges(samples_pca=samples_pca, mean_pca=mean_pca,
                                                           custom_std_ranges=custom_std_ranges,
                                                           num_selected=num_selected_models,
                                                           random_seed=random_seed)
    # 8.4) Visualization: samples in a 2D PCA projection
    selected_indices_dict = {
        f"0-0.5 Std Dev": selected_indices[0],
        f"0.5-1.0 Std Dev": selected_indices[1],
        f"1.0-2.0 Std Dev": selected_indices[2]
    }
    plot_pca_samples_with_ellipses(samples_pca=samples_pca, mean_pca=mean_pca, cov_pca=cov_pca,
                                   selected_indices_dict=selected_indices_dict)
    # 8.5) Visualization: Selected models in one figure
    plot_all_selected_models(R=model_general['R'], num_subs=auxiliary_parameters['num_subs'],
                             samples=posterior_model_samples, selected_indices_dict=selected_indices_dict,
                             num_cols=num_selected_models)
